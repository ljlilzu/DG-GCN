...
def calc_mean_sd(results, metric_names=None):
    if metric_names is None:
        metric_names = ['accuracy']  # 默认值
    ...
    for name in metric_names:
        result_str += f"{name}= {MEAN:.2f}±{SSD:.2f}"
    return result_str
    '''
    # 格式化并打印结果
    for name in metric_names:
        print(f"{name}= {MEAN:.2f}±{SSD:.2f}")

# 计算特征矩阵 X 和 X' 的均方误差损失
def feature_mse_loss(feat, feat_new):
    loss = torch.mean(torch.sum((feat - feat_new) ** 2, dim=1))
    return loss
